...
func _on_EntitySpawner_enemy_respawned(enemy: Enemy) -> void:
	...
	if enemy.stats in _loot_alien_stats and not options.enable_curse_any_enemy:
		return
	...
	if (options.enable_curse_enemies or Utils.get_chance_success(curse_chance / nb_players) or DebugService.always_curse or DebugService.curse_enemy_spawn) and enemy.can_be_cursed:
